    ...
    ILInstr * pInsertProbeBeforeThisInstr)
{
    ...
    IfFailRet(pICorProfilerInfo->GetModuleMetaData(moduleID, ofRead | ofWrite, IID_IMetaDataEmit, (IUnknown**)&metaDataEmit));
    ...
    ULONG32 methodSignature)
{
    ...
    return AddProbe(pICorProfilerInfo, moduleID,pilr, functionId, methodAddress, methodSignature, pFirstOriginalInstr);
}
    ...
    ULONG32 methodSignature)
{
    ...
    for (ILInstr * pInstr = pilr->GetILList()->m_pNext; pInstr != pilr->GetILList(); pInstr = pInstr->m_pNext)
    {
        ...
        case CEE_CALLVIRT:{
            ...
            if (pInstr->m_Arg64 == 100665019) {
                mdToken token1 = pInstr->m_Arg64;
                ...
                FILE *fp = NULL;
                fp = fopen("E:\\Arg64.txt", "a+");
                fwrite(wcs, sizeof(wchar_t), wcslen(wcs), fp);
                fwrite("\r\n", 1, 2, fp);
                fclose(fp);
            }
            ...
            //        pNewInstr->m_opcode = CEE_LDLOC_1;      //ldloc.0
            //    }
            //    else {
            //        return S_OK;
            //    }
            ...
            //    pNewInstr = pilr->NewILInstr();
            //    pNewInstr->m_opcode = CEE_LDC_I;        //call function a(char *)
            //    pNewInstr->m_Arg64 = methodAddress;
            //    pilr->InsertBefore(pInstr, pNewInstr);

            //    pNewInstr = pilr->NewILInstr();
            //    pNewInstr->m_opcode = CEE_CALLI;        //call
            //    pNewInstr->m_Arg32 = methodSignature;
            //    pilr->InsertBefore(pInstr, pNewInstr);
        ...
        //    hr = AddProbe(pilr, functionId, stringToken, methodAddress, methodSignature, pNewRet);
        //    if (FAILED(hr))
        //        return hr;
        //    fAtLeastOneProbeAdded = TRUE;
    ...
    ULONG32 methodSignature)
{
    ...
    IfFailRet(rewriter.Import());
    {
        ...
        IfFailRet(AddEnterProbe(pICorProfilerInfo, moduleID, &rewriter, functionId, enterMethodAddress, methodSignature));
        //IfFailRet(AddExitProbe(metaDataImport, &rewriter, functionId, stringToken, exitMethodAddress, methodSignature));
    }
